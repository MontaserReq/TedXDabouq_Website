...
export const login = async (values: z.infer<typeof LoginSchema>) => {
  ...
  if (existingUser.lockoutUntil && new Date(existingUser.lockoutUntil) > new Date()) {
    const remainingTime = Math.ceil((new Date(existingUser.lockoutUntil).getTime() - new Date().getTime()) / 1000);
    return { error: `Account is locked. Try again in ${remainingTime} seconds.` };
  }
